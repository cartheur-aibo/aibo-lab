...
usage() {
  cat <<'EOF'
usage: prepare-ers7-sdk-wconsole-objectcomm.sh [/path/to/output-dir]

Prepare a pure OPEN-R SDK ERS-7 WCONSOLE/nomemprot stick using the local
ObjectComm sample, matching the official Remote Processing OPEN-R AIBO-side
layout.
EOF
}
...
WORK_SAMPLE_DIR="$WORK_ROOT/ObjectComm"
...
cp -a "$WORK_SAMPLE_DIR/MS/OPEN-R/MW/CONF/OBJECT.CFG" "$OUTPUT_DIR/OPEN-R/MW/CONF/OBJECT.CFG"
cp -a "$WORK_SAMPLE_DIR/MS/OPEN-R/MW/CONF/CONNECT.CFG" "$OUTPUT_DIR/OPEN-R/MW/CONF/CONNECT.CFG"
...
ERS-7 SDK ObjectComm copy notes

SDK base: $SDK_BASE_DIR
Sample: $SAMPLE_DIR
Stick staging dir: $OUTPUT_DIR

This is the AIBO-side WCONSOLE/nomemprot stick for the ObjectComm Remote
Processing sample.
...
5. If using the host-side RP flow, build and run the host program separately.
EOF

SIZE_BYTES="$(du -sb "$OUTPUT_DIR" | awk '{print $1}')"
SIZE_MIB="$(awk -v bytes="$SIZE_BYTES" 'BEGIN { printf "%.2f", bytes / (1024 * 1024) }')"

echo "Prepared ERS-7 SDK ObjectComm stick at: $OUTPUT_DIR"
